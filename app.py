...
st.markdown("""
    ...
    .idx-badge { background-color: #343a40; color: white; padding: 2px 10px; border-radius: 6px; font-size: 0.9em; margin-right: 12px; font-weight: 600; }
    .k-num-text { font-size: 1.3em; font-weight: 800; color: #000; }
    .label { font-weight: 700; color: #555; min-width: 100px; display: inline-block; }
    </style>
...
def run_query(kn, k1, k2, lmt):
    ...
    with st.spinner('🔍 正在檢索 FDA 資料庫並驗證 PDF 檔案路徑...'):
        try:
            ...
            for r in raw_data:
                ...
                processed_results.append({
                    "k_num": k,
                    "class_name": r.get('device_name', '未知'), # OpenFDA 中 device_name 通常是分類名
                    "trade_name": r.get('proprietary_name', '未提供產品名稱'),
            ...
            st.success(f"✅ 找到 {len(processed_results)} 筆資料 (已將可下載 PDF 的結果優先顯示)")
            ...
            for i, item in enumerate(processed_results, 1):
                ...
                pdf_btn = f'<a href="{item["pdf_url"]}" target="_blank" style="color: #ffffff; background-color: #d9534f; padding: 4px 12px; border-radius: 4px; text-decoration: none; font-size: 0.85em; font-weight: 600;">📄 下載 PDF</a>' if item['is_ok'] else ""

                # 建構扁平化 HTML (解決 </div> 殘留問題)
                html_card = (
                    ...
                    f'</div>'
                    f'<div style="margin-bottom: 8px;"><span class="label">分類名稱：</span>{item["class_name"]}</div>'
                    f'<div style="margin-bottom: 8px;"><span class="label">產品名稱：</span>{item["trade_name"]}</div>'
                    f'<div style="margin-bottom: 15px;"><span class="label">申請廠商：</span>{item["applicant"]}</div>'
                    f'<div style="display: flex; gap: 15px; align-items: center; border-top: 1px solid #eee; padding-top: 15px;">'
