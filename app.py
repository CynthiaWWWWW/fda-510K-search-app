...
# --- 2. CSS 樣式設定 ---
# 透過 Markdown 注入自定義 CSS，美化標題、資訊文字、結果卡片與標籤
st.markdown("""
    <style>
    ...
    <div class="info-text">連線 OpenFDA 資料庫進行精確欄位篩選（支援模糊搜尋）</div>
...
def run_query(kn, k1, k2, app, lmt):
    ...
    with st.spinner('正在根據篩選條件檢索 FDA 資料庫...'):
        try:
            ...
            for r in raw_data:
                k = r.get('k_number')
                # 根據號碼規則預測 PDF 存放路徑（例：K231234 -> pdf23/K231234.pdf）
                pdf = f"https://www.accessdata.fda.gov/cdrh_docs/pdf{k[1:3]}/{k}.pdf"
                
                # 檢查 PDF 檔案是否存在（發送 HEAD 請求節省頻寬）
                try:
